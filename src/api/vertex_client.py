        ...
        for attempt in range(max_retries + 1):
            ...
            for i, msg in enumerate(messages):
                if msg["role"] == "user":
                    ...
                    if (
                        assistant_content
                        and "data:image/" in assistant_content
                        and ";base64," in assistant_content
                    ):
                        _, image_parts = extract_images_from_assistant_message(
                            assistant_content
                        )
                        if image_parts:
                            for img_part in image_parts:
                                all_assistant_images_with_turn.append(
                                    (assistant_turn_number, img_part)
                                )
            ...
            for i, msg in enumerate(messages):
                if msg["role"] == "system":
                    ...
                    if i == last_user_index and all_assistant_images_with_turn:
                        parts.append(
                            {
                                "text": f"[以下是之前生成的 {len(all_assistant_images_with_turn)} 张图片：]"
                            }
                        )
                        current_turn = 0
                        ...
                        print(
                            f"ℹ️ 注入 {len(all_assistant_images_with_turn)} 张历史图片"
                        )
                    ...
                    if (
                        ...
                        cleaned_text, _ = extract_images_from_assistant_message(
                            assistant_content
                        )

                        if cleaned_text.strip():
                            chat_history.append(
                                {"role": "model", "parts": [{"text": cleaned_text}]}
                            )
                        else:
                            # 如果没有文本，添加一个简短说明
                            chat_history.append(
                                {"role": "model", "parts": [{"text": "[已生成图片]"}]}
                            )
                    else:
                        # 普通文本消息，直接添加
            ...
            if "image" in target_model:
                ...
                if "imageOutputOptions" not in gen_config["imageConfig"]:
                    gen_config["imageConfig"]["imageOutputOptions"] = {
                        "mimeType": "image/png"
                    }

                # Only add imageSize if a resolution suffix is present
                if resolution_mode:
                    size_str_map = {"1k": "1K", "2k": "2K", "4k": "4K"}
                    if resolution_mode in size_str_map:
                        gen_config["imageConfig"]["imageSize"] = size_str_map[
                            resolution_mode
                        ]
                        print(
                            f"ℹ️ 图像生成: 尺寸={gen_config['imageConfig']['imageSize']}"
                        )
                else:
            ...
            # Reassemble body
            new_body = {
                "querySignature": original_body.get(
                    "querySignature"
                ),  # Might need this?
            ...
            headers = creds[
                "headers"
            ].copy()  # Copy to avoid mutating the cached credentials
            ...
            try:
                ...
                async with self._create_isolated_client() as client:
                    ...
                    if response.status_code != 200:
                        ...
                        if (
                            ...
                            print(
                                f"⚠️ 认证错误 ({response.status_code})，尝试刷新并重试..."
                            )
                            ...
                            # 等待一小段时间让刷新生效（虽然不阻塞等待完成，但给点时间）
                            await asyncio.sleep(2)

                            # 重新加载凭证
                            self.cred_manager.load_from_disk()
                    ...
                    aggregator = ChunkAggregator(
                        min_chunk_size=256, max_buffer_time=0.1
                    )
                    ...
                    try:
                        ...
                        async for sse_event in stream_processor.process_stream(
                            stabilized_stream, model=model
                        ):
                            # 检查是否有资源耗尽错误
                    ...
                    if stream_error:
                        if content_yielded:
                            ...
                            error_payload = {
                                "error": {
                                    "message": f"Authentication failed mid-stream: {str(stream_error)}",
                        ...
                        if attempt < max_retries:
                            print(
                                f"🔄 认证错误，尝试刷新并重试 (尝试 {attempt+1}/{max_retries+1})..."
                            )
                        ...
                        # 重试用尽或刷新失败 - 返回友好错误信息
                        error_msg = (
                            "认证已过期，请刷新浏览器中的 Vertex AI Studio 页面后重试"
                        )
                        error_payload = {
                            "error": {
                                "message": error_msg,
                    ...
                    if is_image_model:
                        ...
                        # LLM: 根据实际内容估算
                        prompt_tokens = self.stats_manager.estimate_messages_tokens(
                            messages
                        )
                        completion_tokens = (
                            max(1, int(total_completion_chars / 3.5))
                            if total_completion_chars > 0
                            else 1
                        )

                    await self.stats_manager.update(prompt_tokens, completion_tokens)
                    self.stats_manager.set_current_request_tokens(
                    ...
                    if is_image_model:
                        ...
                        print(
                            f"✅ {chunk_count} 块 | {prompt_tokens}+{completion_tokens}={prompt_tokens+completion_tokens} token"
                        )
                ...
                if content_yielded:
                    ...
                    error_payload = {
                        "error": {
                            "message": f"Authentication failed mid-stream: {str(e)}",
                            "type": "authentication_error",
                        }
                    }
                    yield f"data: {json.dumps(error_payload)}\n\n"
